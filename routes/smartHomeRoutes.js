...
router.post(
    ...
    async (req, res) => {
        try {
            const { pairingCode, esp32Id } = req.body;
            ...
            const normalizedPairingCode = pairingCode.toUpperCase().trim();
            const normalizedESP32Id = esp32Id.trim();
            ...
            // CONNECT ESP32
            // -------------------------------------
            smartHome.esp32Id = normalizedESP32Id;
            smartHome.status = "connected";
            smartHome.lastSeen = new Date();
            ...
            console.log("==============================");
            console.log("ESP32 ID:", normalizedESP32Id);
            console.log("Smart Home:", smartHome.name);
            console.log("Smart Home ID:", smartHome._id);
            console.log("==============================");
            ...
            res.json({
                ...
                smartHome: {
                    ...
                    pumpIsOn: smartHome.pumpIsOn,
                    lastSeen: smartHome.lastSeen,
                },
            });
        } catch (error) {
            console.error("ESP32 Pairing Error:", error);
            res.status(500).json({
                success: false,
                message: "Server error",
            });
...
router.get(
    ...
    async (req, res) => {
        try {
            ...
            const smartHome = await getUserSmartHome(userId);
...
// Replace static status check with the dynamic 35-second threshold:
let isConnected = false;
if (smartHome.esp32Id && smartHome.lastSeen) {
    const difference = Date.now() - new Date(smartHome.lastSeen).getTime();
    isConnected = difference <= 35000;
}

const computedStatus = isConnected ? "connected" : "disconnected";

            // If database status is stale, sync it
            if (smartHome.status !== computedStatus) {
                smartHome.status = computedStatus;
                await smartHome.save();
            }
            ...
            res.json({
                ...
                smartHome: {
                    ...
                    // SENSOR DATA
                    // =================================
                    temperature: smartHome.temperature ?? null,
                    humidity: smartHome.humidity ?? null,
                    doorStatus: smartHome.doorStatus ?? null,
                    sensorLastUpdated: smartHome.sensorLastUpdated ?? null,
                },
            });
        } catch (error) {
            console.error("Get Smart Home Error:", error);
            res.status(500).json({
                success: false,
                message: "Server error",
            });
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            console.log(`Smart Home created: ${smartHome.name}`);
            ...
            console.error("Create Smart Home Error:", error);
            res.status(500).json({
                success: false,
                message: "Server error",
            });
...
router.get(
    ...
    async (req, res) => {
        try {
            ...
            // -------------------------------------
            // Check heartbeat (35-sec threshold)
            // -------------------------------------
            let isConnected = false;
            ...
            // Update status
            // -------------------------------------
            smartHome.status = isConnected ? "connected" : "disconnected";
            await smartHome.save();
            ...
            res.json({
                ...
                lastSeen: smartHome.lastSeen || null,
            });
        } catch (error) {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            smartHome.lastSeen = null;
            smartHome.pairingCode = null;
            smartHome.pairingCodeExpiresAt = null;

            await smartHome.save();

            console.log("ESP32 unpaired successfully");
            ...
            console.error("Unpair ESP32 Error:", error);
            res.status(500).json({
                success: false,
                message: "Server error",
            });
...
router.post(
    ...
    async (req, res) => {
        try {
            console.log();
            console.log("=================================");
            console.log("ESP32 HEARTBEAT REQUEST RECEIVED");
            console.log("=================================");
            ...
            if (!smartHome) {
                console.log(`Unknown ESP32 heartbeat: ${normalizedESP32Id}`);
                return res.status(404).json({
                    success: false,
                    message: "ESP32 is not paired",
                });
            }
            ...
            console.log(`Heartbeat received from ${normalizedESP32Id}`);
            console.log(`Last seen: ${smartHome.lastSeen}`);
            console.log("=================================");
            ...
            console.error("ESP32 Heartbeat Error:", error);
            res.status(500).json({
                success: false,
                message: "Server error",
            });
